...
class DatabaseService {
  ...
  final CollectionReference salleCollection = FirebaseFirestore.instance.collection('Salle'); 
  final CollectionReference etageCollection =  FirebaseFirestore.instance.collection('Etage'); 

  //Récupération de tous les étages 
  Future<List<Etage>> recuperationEtages() async { 
    List <Etage> listEtages=[]; 
    QuerySnapshot etagesSnapshot = await FirebaseFirestore.instance.collection('Etage').get(); 
...
//Récupération des salles rattachées à l'id de l'étage passé en paramètre
  Future<List<Salle>> recuperationSalles(String idEtage) async{   
    List <Salle> salles=[];  
    QuerySnapshot sallesSnapshot = await salleCollection.where('id_etage',isEqualTo:idEtage).get();
...
//Récupération des commentaires fait sur une salle dont l'id est passé en paramètre
  Future<List<Commentaire>> recuperationCommentaire(String idClasse) async{  
    ...
    QuerySnapshot commentaireSnapshot = await FirebaseFirestore.instance.collection('Commentaire').where('id_salle',isEqualTo: idClasse).get();  
  ...
  // Ajout des commentaires dans la  base de données  
     Future<void> ajoutCommentaire(String idSalle, String contenuCommentaire)async { 

      try{
      if(idSalle.isNotEmpty ){ 
      Commentaire com=Commentaire(id:"", contenu: contenuCommentaire, dateCommentaire: DateTime.now());
         Map <String, dynamic> commentaireJson= com.toJson(idSalle); 
         CollectionReference commentaireCollection = FirebaseFirestore.instance.collection("Commentaire"); 
